...
def main():
    ...
    print("[*] Getting CNN probabilities on validation set...")
    probs = cnn.predict(X_val, verbose=0).reshape(-1)
    ...
    # 2) Brier score (overall calibration error)
    brier = brier_score_loss(y_val, probs)
    print(f"\nBrier score (lower is better): {brier:.4f}")

    # 3) High-confidence region used by your override
