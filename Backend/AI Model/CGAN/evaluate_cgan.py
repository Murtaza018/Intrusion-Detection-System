...
# Configuration
MODEL_NAME = "cgan_generator.keras"
NUM_SAMPLES = 10
...
def evaluate_generator():
    ...
    if label_dim > 1:
        # Assuming Class 0 is "Normal" (usually the first class)
        # Create a matrix of zeros [10, 15]
        labels = np.zeros((NUM_SAMPLES, label_dim))
        # Set the first column to 1 (representing Class 0)
        labels[:, 0] = 1 
        print(f"    -> Created One-Hot Encoded labels for Class 0 (Normal)")
    else:
        # Binary classification (0 = Normal)
